...
class individualCamera(QMainWindow):
    ...
    def captureAndDisplayImage(self):
        ...
        image_SIZE = self.setCameraResolution(640, 480)
        saved_image_SIZE = self.setCameraResolution(3264, 2448)
        if self.camera is not None:
            ret, captured_image = self.camera.read()
            frameShow = cv2.resize(captured_image, image_SIZE)
            ...
            if ret:
                ...
                cv2.imwrite(image_filename, saved_image_SIZE)
    ...
    def updateFrame(self):
        ...
        #cropCoordinates = (240, 200, 400, 280)
